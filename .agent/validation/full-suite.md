...
- `$PDF_DOC_ID` - ID of uploaded PDF document
- `$DOCX_DOC_ID` - ID of uploaded DOCX document
- `$HTML_DOC_ID` - ID of uploaded HTML document
- `$HYBRID_DOC_ID` - ID of uploaded hybrid search test document
...
#### API-41: Upload duplicate file returns existing document
**Steps:**
```bash
# Upload test_document.txt first time
curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.txt" | jq -r '.id'

# Store the document ID
DUP_DOC_ID=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.txt" | jq -r '.id')

# Upload same file again
DUP_DOC_ID_2=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.txt" | jq -r '.id')
...
echo "$DUP_DOC_ID_2"
```
**Acceptance Criteria:** Both IDs are identical (same document returned, no duplicate created).

---

#### API-42: Upload modified file updates existing document
**Steps:**
```bash
# Upload original file
ORIG_DOC_ID=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.txt" | jq -r '.id')

# Get original content_hash
ORIG_HASH=$(curl -s http://localhost:8000/documents \
  -H "Authorization: Bearer $TOKEN1" | jq -r ".[] | select(.id==\"$ORIG_DOC_ID\") | .content_hash")

# Create modified version
echo "Original content\nNew line added" > /tmp/test_modified.txt

# Upload modified file with same name
UPDATED_DOC_ID=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@/tmp/test_modified.txt" | jq -r '.id')

# Get new content_hash
NEW_HASH=$(curl -s http://localhost:8000/documents \
  -H "Authorization: Bearer $TOKEN1" | jq -r ".[] | select(.id==\"$UPDATED_DOC_ID\") | .content_hash")

echo "Original ID: $ORIG_DOC_ID"
echo "Updated ID: $UPDATED_DOC_ID"
echo "Original hash: $ORIG_HASH"
echo "New hash: $NEW_HASH"
```
**Acceptance Criteria:**
- Same document ID returned
- `content_hash` is different
- Document transitions to "processing" then "completed"

---

#### API-43: Upload same content with different filename returns existing
**Steps:**
```bash
# Upload original
ORIG_DOC=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.txt" | jq -r '.id')

# Copy content to new file
cp .agent/validation/fixtures/test_document.txt /tmp/test_document_copy.txt

# Upload copy with different filename
COPY_DOC=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@/tmp/test_document_copy.txt" | jq -r '.id')

echo "Original: $ORIG_DOC"
echo "Copy: $COPY_DOC"
```
**Acceptance Criteria:** Both IDs are identical (same content_hash detected, existing document returned).

---

#### API-44: Upload document with metadata extraction
**Steps:**
```bash
# Upload a Python-related document
DOC_ID=$(curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_rag_document.txt" | jq -r '.id')
...
curl -s http://localhost:8000/documents \
  -H "Authorization: Bearer $TOKEN1" | jq ".[] | select(.id==\"$DOC_ID\") | .metadata"
```
**Acceptance Criteria:**
...
#### API-45: Search with metadata filters
**Steps:**
```bash
# Upload two different types of documents and wait for completion
# (Assume documents already exist from previous tests)

# Search without filters - general query
curl -s -X POST http://localhost:8000/threads/$THREAD_ID/messages \
  -H "Authorization: Bearer $TOKEN1" \
  -H "Content-Type: application/json" \
  -d '{"content":"Find programming tutorials"}' > /dev/null

# Check LangSmith traces for tool calls with metadata_filters
```
**Acceptance Criteria:**
- LLM calls search_documents tool
- Tool call may include metadata_filters parameter when query mentions specific types
- Results are filtered accordingly when filters are present

---

#### API-46: Metadata inheritance to chunks
**Steps:**
```bash
# Get document ID from previous test
DOC_ID="<from-previous-test>"

# Query chunks for this document via database or check during retrieval
# This requires direct database access or examining search results
```
**Acceptance Criteria:**
- All chunks have `metadata` field populated
- Chunk metadata includes document-level fields (document_type, topics, etc.)
- Chunk metadata includes chunk-specific fields (filename, chunk_index)
- Metadata is identical across all chunks from same document (excluding chunk_index)
...
**Acceptance Criteria:**
- HTTP 201.
- Response contains `"filename":"test_document.pdf"`, `"file_type":"application/pdf"`, `"status":"pending"`.
- Save `id` as `$PDF_DOC_ID`.
- Wait for status to transition to `"completed"` with `chunk_count` > 0.
...
curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/test_document.docx"
```
**Acceptance Criteria:**
- HTTP 201.
- Response contains `"filename":"test_document.docx"`, `"file_type":"application/vnd.openxmlformats-officedocument.wordprocessingml.document"`, `"status":"pending"`.
- Save `id` as `$DOCX_DOC_ID`.
- Wait for status to transition to `"completed"` with `chunk_count` > 0.
...
**Acceptance Criteria:**
- HTTP 201.
- Response contains `"filename":"test_document.html"`, `"file_type":"text/html"`, `"status":"pending"`.
- Save `id` as `$HTML_DOC_ID`.
- Wait for status to transition to `"completed"` with `chunk_count` > 0.

---

#### API-50: Reject unsupported file type (.xlsx)
**Steps:**
```bash
# Create a temporary .xlsx file (or use another unsupported extension)
echo "fake spreadsheet data" > /tmp/test_invalid.xlsx
curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@/tmp/test_invalid.xlsx"
```
**Acceptance Criteria:** HTTP status code is `400`. Error message indicates unsupported file type.
...
CHUNK_COUNT_BEFORE=$(curl -s http://localhost:8000/documents \
  -H "Authorization: Bearer $TOKEN1" | jq ".[] | select(.id==\"$HTML_DOC_ID\") | .chunk_count")

echo "Chunks before delete: $CHUNK_COUNT_BEFORE"

# Delete document
curl -s -X DELETE http://localhost:8000/documents/$HTML_DOC_ID \
  -H "Authorization: Bearer $TOKEN1"

# Verify document deleted
curl -s http://localhost:8000/documents \
  -H "Authorization: Bearer $TOKEN1" | jq ".[] | select(.id==\"$HTML_DOC_ID\")"
```
**Acceptance Criteria:**
- Document had `chunk_count` > 0 before deletion
- After deletion, document not found in list
- Chunks are automatically removed (verify via database or absence of orphaned chunks in subsequent searches)
...
curl -s -X POST http://localhost:8000/documents/upload \
  -H "Authorization: Bearer $TOKEN1" \
  -F "file=@.agent/validation/fixtures/hybrid_search_test.txt"
```
**Acceptance Criteria:**
- HTTP 201.
- Response contains `"filename":"hybrid_search_test.txt"`, `"status":"pending"`.
- Save `id` as `$HYBRID_DOC_ID`.
- Wait for status to transition to `"completed"` with `chunk_count` > 0.

---

#### API-53: Test hybrid search with keyword query
**Steps:**
```bash
# This should find exact matches for "CORS" (keyword search component)
curl -s -N -X POST http://localhost:8000/threads/$THREAD_ID/messages \
  -H "Authorization: Bearer $TOKEN1" \
  -H "Content-Type: application/json" \
  -d '{"content":"What is CORS configuration in FastAPI?"}' \
  --max-time 60
```
**Acceptance Criteria:**
- SSE stream contains text mentioning "CORS" and "middleware"
- Response references the hybrid_search_test document content
- Keyword search component should boost exact "CORS" matches

---

#### API-54: Test hybrid search with semantic query
**Steps:**
```bash
# This should use vector search to find semantically similar content
curl -s -N -X POST http://localhost:8000/threads/$THREAD_ID/messages \
  -H "Authorization: Bearer $TOKEN1" \
  -H "Content-Type: application/json" \
  -d '{"content":"How do I optimize database connections in FastAPI?"}' \
  --max-time 60
```
**Acceptance Criteria:**
- SSE stream contains text about connection pooling, pool_size, max_overflow
- Response references the hybrid_search_test document
- Vector search component finds semantically related content even without exact keyword matches

---

#### API-55: Test hybrid search with mixed query
**Steps:**
```bash
# This query combines specific terms (FastAPI, REDIS) with semantic concepts (performance)
curl -s -N -X POST http://localhost:8000/threads/$THREAD_ID/messages \
  -H "Authorization: Bearer $TOKEN1" \
  -H "Content-Type: application/json" \
  -d '{"content":"How to improve FastAPI performance with REDIS caching?"}' \
  --max-time 60
```
**Acceptance Criteria:**
- SSE stream contains relevant information about Redis caching and performance
- Both keyword matching (FastAPI, REDIS) and semantic search (performance, caching) contribute
- RRF fusion combines results effectively
- Reranking should surface most relevant chunks
...
#### E2E-28: Duplicate upload behavior
**Steps:**
1. Sign in as test@test.com (admin)
2. Navigate to Documents page
3. `browser_snapshot` to verify starting state
4. Upload `test_rag_document.txt` via drag-and-drop or file picker
5. `browser_wait_for` time: 10 seconds (wait for status → completed)
6. `browser_snapshot` (note document count)
7. Upload the exact same file again (`test_rag_document.txt`)
8. `browser_wait_for` time: 3 seconds
9. `browser_snapshot`
**Acceptance Criteria:**
- File re-uploaded successfully with no error
- No duplicate shown in document list (count remains same)
- Document still shows "completed" status (no re-processing)
- Upload completes immediately (deduplication detected)

---

#### E2E-29: Metadata display in UI
**Steps:**
1. Sign in as test@test.com (admin)
2. Navigate to Documents page
3. Upload a technical document (e.g., `test_rag_document.txt`)
4. `browser_wait_for` time: 15 seconds (wait for metadata extraction and processing)
5. `browser_snapshot` to capture document card with metadata
6. Observe metadata badges on document card
**Acceptance Criteria:**
- Metadata badges appear below document info (document type, topics, languages)
- Badges are color-coded and readable
- Document type badge shows (e.g., "tutorial", "guide", "documentation")
- Topics appear as green badges (up to 3 shown)
- Programming languages appear as purple badges (if detected)
- Frameworks/tools appear as orange badges (if detected)
- Technical level badge shows if not "intermediate"
- Badges only appear for completed documents
...
1. Sign in as test@test.com (admin)
2. Navigate to Documents page
3. `browser_snapshot` to verify upload zone accepts multiple formats
4. Upload `test_document.pdf` via file picker or drag-and-drop
5. `browser_wait_for` time: 2 seconds
6. `browser_snapshot` (should show PDF with 📄 icon)
7. Upload `test_document.docx`
8. `browser_wait_for` time: 2 seconds
9. `browser_snapshot` (should show DOCX with 📝 icon)
10. Upload `test_document.html`
11. `browser_wait_for` time: 2 seconds
12. `browser_snapshot` (should show HTML with 🌐 icon)
13. `browser_wait_for` time: 20 seconds (wait for all to process)
14. `browser_snapshot` to capture final state
**Acceptance Criteria:**
- All three documents appear in document list with correct file type icons
- PDF shows 📄 icon
- DOCX shows 📝 icon
- HTML shows 🌐 icon
- All documents transition to "completed" status
- All documents show `chunk_count` > 0
- Metadata badges appear for all completed documents
- No error messages displayed

---

#### E2E-31: Hybrid search in chat with keyword and semantic queries
**Steps:**
1. Sign in as test@test.com (admin)
2. Navigate to Documents page
3. Upload `hybrid_search_test.txt` via file picker
4. `browser_wait_for` time: 15 seconds (wait for processing and metadata extraction)
5. Navigate to Chat page
6. Create a new thread
7. Type message: `How to configure CORS in FastAPI?`
8. Send the message
9. `browser_wait_for` time: 30 seconds (allow tool calling + response)
10. `browser_snapshot` to capture response
11. Verify response mentions CORS configuration and middleware
12. Type message: `What are FastAPI performance optimization techniques?`
13. Send the message
14. `browser_wait_for` time: 30 seconds
15. `browser_snapshot` to capture response
**Acceptance Criteria:**
- First query (keyword-focused) finds exact CORS information from the document
- Second query (semantic) finds performance optimization content even without exact keyword matches
- Both responses cite the hybrid_search_test document as source
- Hybrid search (vector + keyword + RRF + reranking) provides relevant results
- Chat UI displays document citations correctly
