-- Fix type mismatch in match_chunks and keyword_search_chunks functions
-- Issue: Column 6 (similarity) and column 7 (rank) were returning real instead of double precision
-- Cause: The cosine distance operator (<=>)  and ts_rank return real (single precision)
-- Solution: Cast to double precision explicitly
