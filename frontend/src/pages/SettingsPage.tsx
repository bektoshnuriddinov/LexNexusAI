...
export function SettingsPage() {
  ...
  const [jinaRerankModel, setJinaRerankModel] = useState('')
  const [jinaRerankEnabled, setJinaRerankEnabled] = useState(false)
  ...
  async function loadSettings() {
    ...
    try {
      ...
      setJinaRerankModel(settings.jina_rerank_model || 'jina-reranker-v2-base-multilingual')
      setJinaRerankEnabled(settings.jina_rerank_enabled || false)
    } catch (err) {
  ...
  async function handleSave() {
    ...
    try {
      const update: GlobalSettingsUpdate = {
        ...
        // Reranker settings
        jina_api_key: jinaApiKey || null,
        jina_rerank_model: jinaRerankModel || null,
        jina_rerank_enabled: jinaRerankEnabled,
      }
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex w-64 flex-col border-r bg-muted/30">
        <div className="border-b p-6 flex items-center justify-center bg-gradient-to-br from-background to-muted/20">
          <img
            src={logoPng}
            alt="LexNexus AI"
        ...
        <div className="max-w-lg mx-auto p-8">
          ...
          {loading ? (
            ...
            <form autoComplete="off" onSubmit={(e) => e.preventDefault()} className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-medium text-foreground">Reranker Configuration (Optional)</h3>
                <p className="text-xs text-muted-foreground">
                  Enable reranking for improved search relevance. Sign up at{' '}
                  <a
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      checked={jinaRerankEnabled}
                      onChange={(e) => setJinaRerankEnabled(e.target.checked)}
                      className="h-4 w-4"
                    />
                    <label htmlFor="jina_rerank_enabled" className="text-sm">
                      Enable Reranking
                    </label>
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs text-muted-foreground">Jina API Key</label>
                    <div className="relative">
                      <Input
                        name="jina-key-field"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs text-muted-foreground">Reranker Model</label>
                    <Input
                      name="jina-model-field"
                      autoComplete="off"
                      placeholder="e.g., jina-reranker-v2-base-multilingual"
                      value={jinaRerankModel}
                      onChange={(e) => setJinaRerankModel(e.target.value)}
                    />
                  </div>
                </div>
              </div>
